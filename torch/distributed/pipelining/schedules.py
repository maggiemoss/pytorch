...
def _validate_pipeline_order(
    ...
    for timestep in range(max_timestep):
        ...
        if len(unique_microbatch_indices) != len(current_timestep_actions):
            error_msg.append(
                f"Duplicate microbatch index found in current_timestep_actions"
            )
            ...
            for action in pipeline_order[rank]:
                ...
                if computation_type == _ComputationType.BACKWARD:
                    ...
                    if (mb_index, stage_index) not in backward_steps:
                        error_msg.append(f"{mb_index=}, {stage_index=} Weight happened before bwd")
                    if (mb_index, stage_index) in weight_steps:
                        error_msg.append(f"{mb_index=}, {stage_index=} Duplicated weight step")
...
class ScheduleFlexibleInterleaved1F1B(PipelineScheduleMulti):
    ...
    def _calculate_single_rank_operations(self, rank) -> List[Optional[_Action]]:
        def get_rank_warmup_ops(rank):
            ...
            warmup_ops = warmups_ops_last_stage + multiply_factor * ((self.pp_group_size - 1) - rank)
    ...
    def _add_bubbles_to_actions(self, num_stages_global):
        ...
        while True:
            ...
            for rank in range(self.pp_group_size):
                ...
                if not need_bubble(stage_index, op, microbatch, num_stages_global, seen_ops):
        ...
        if total_bubbles_added > 0:
            logger.warning("Non zero bubbles added: {total_bubbles_added=} {bubbles_added=}")
        return result
