...
def module_error_inputs_torch_nn_BatchNorm1d_2d_3d(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    make_input = partial(
        make_tensor, device=device, dtype=dtype, requires_grad=requires_grad
...
def module_error_inputs_torch_nn_GroupNorm(
    module_info, device, dtype, requires_grad, training, **kwargs
):
    """
...
def module_error_inputs_torch_nn_RNN_GRU(
    ...
    make_input = partial(
        make_tensor, device=device, dtype=mismatched_dtype, requires_grad=requires_grad
    )
    ...
    samples = [
        ...
        ErrorModuleInput(
            ...
            error_type=ValueError,
            error_regex=(
                r"RNN input dtype .* does not match weight dtype .* "
