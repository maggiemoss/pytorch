...
ROCM_VERSION = LazyVal(
    lambda: tuple(int(v) for v in torch.version.hip.split(".")[:2])
...
IS_THOR = LazyVal(
    ...
    and (
        (
            ...
            and int(torch.version.cuda[:2]) >= 13
        )
        or (
            torch.cuda.get_device_capability() == (10, 1)
            and int(torch.version.cuda[:2]) < 13
        )
    )
)
...
requires_triton_ptxas_compat = unittest.skipIf(
    torch.version.hip is None and _get_torch_cuda_version() < TRITON_PTXAS_VERSION,
