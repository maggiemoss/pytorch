...
class TestModule(TestCase):
            ...
            with freeze_rng_state():
                ...
                if torch.accelerator.device_count() >= 2:
                    ...
                    self._assert_module_parameters_and_buffer_are(
                        m, torch.device(f"{device_type}:1"), dtype
                    )
    ...
    def _test_gradients_helper(self, device, dtype, module_info, training, check):
        ...
        if "xpu" in device and module_info.name == "nn.MultiheadAttention":
            self.skipTest(
                "GradcheckError issue in MultiheadAttention, https://github.com/intel/torch-xpu-ops/issues/2356"
            )
        # === Set nondet tol for gradcheck to user-defined value if on CUDA and cudNN is enabled
        gradcheck_nondet_tol = 0.0
        if (
            torch.device(device).type == "cuda" and torch.backends.cudnn.enabled
        ) or device_type == "xpu":
