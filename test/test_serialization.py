        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            def mock_get_storage(*args, **kwargs):
                raise AssertionError(
                    "get_storage_from_record should not be called with map_location='meta'"
                )

            with patch.object(
                torch._C.PyTorchFileReader, "get_storage_from_record", mock_get_storage
            ):
            ...
            self.assertEqual(
                sd_meta["weight"].untyped_storage().nbytes(),
                sd["weight"].untyped_storage().nbytes(),
            )
            self.assertEqual(
                sd_meta["bias"].untyped_storage().nbytes(),
                sd["bias"].untyped_storage().nbytes(),
            )
...
class TestSerialization(TestCase, SerializationMixin):
    ...
    def test_storage_alignment(self, load_mode):
        ...
        def check_loaded_state_dict(
            sd_loaded, sd_orig, expected_weight_offset, expected_bias_offset
        ):
            ...
            self.assertEqual(
                sd_loaded["weight"].untyped_storage()._checkpoint_offset,
                expected_weight_offset,
            )
            self.assertEqual(
                sd_loaded["bias"].untyped_storage()._checkpoint_offset,
                expected_bias_offset,
            )

            self.assertEqual(
                sd_loaded["weight"].untyped_storage().nbytes(),
                sd_orig["weight"].untyped_storage().nbytes(),
            )
            self.assertEqual(
                sd_loaded["bias"].untyped_storage().nbytes(),
                sd_orig["bias"].untyped_storage().nbytes(),
            )
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            check_loaded_state_dict(
                sd_loaded, sd, expected_weight_offset=832, expected_bias_offset=1344
            )
        ...
        with tempfile.NamedTemporaryFile() as f:
            try:
                ...
                check_loaded_state_dict(
                    sd_loaded,
                    sd,
                    expected_weight_offset=20480,
                    expected_bias_offset=24576,
                )
