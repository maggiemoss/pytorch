    ...
    def test_qtopk(self):
        ...
        for x_dim, side, dim, large, sort, dtype, nhwc in test_cases:
            ...
            unquantized_out = torch.topk(
                qX.dequantize(), k, dim=dim, largest=large, sorted=sort
            )
...
class TestQuantizedLinear(TestCase):
    ...
    def test_qlinear_gelu_pt2e(self):
        ...
        self._test_qlinear_pt2e_helper(
            qlinear, "gelu", post_op_algorithms=post_op_algorithms, test_fast_path=True
        )
...
class TestQuantizedConv(TestCase):
    ...
    def _make_qconv_tensors_fp8(
        ...
        bias = (
            torch.randn((output_channels,), dtype=bias_dtype, device=device)
            if use_bias
            else None
        )
