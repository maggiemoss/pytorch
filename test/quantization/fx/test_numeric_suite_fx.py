...
class TestFXNumericSuiteCoreAPIs(FXNumericSuiteQuantizationTestCase):
    ...
    def test_extend_logger_results_with_comparison(self):
        ...
        for layer_results in results.values():
            assert "sqnr_int8_vs_fp32" in layer_results["weight"]["int8"][0]
            assert "l2_error_int8_vs_fp32" in layer_results["weight"]["int8"][0]
            assert (
                "cosine_similarity_int8_vs_fp32" in layer_results["weight"]["int8"][0]
            )
