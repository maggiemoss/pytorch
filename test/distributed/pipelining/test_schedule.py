...
from model_registry import ModelWithKwargs, MultiMLPWithDw, MultiMLP
...
class ScheduleTest(MultiProcContinousTest):
    ...
    def test_schedule_with_weight_update_mlp_e2e(self, ScheduleClass):
        ...
        class CustomState:
            ...
            def dw_builder(self):
                def dw_runner():
                    ...
                    self.i += 1
                    print(f"[Rank {self.rank}] dw_count={self.i} stage={self.stage_idx}")
        ...
        # Attach to a schedule
        schedule = ScheduleClass(stages, chunks, loss_fn=full_loss_fn,enable_zero_bubble=True)
    ...
    def test_pipeline_order_flex_and_zero_bubble(self, ScheduleClass):
        for num_local_stages, num_microbatches, group_size in self.test_cases:
            with self.subTest(
                ...
                #skip_zero_bubble = num_local_stages * num_microbatches <= warmup_ops
                ...
                for i in range(2):
                    ...
                    schedule = ScheduleClass(stages, num_microbatches, enable_zero_bubble=(i == 0))
                    formatted_pipeline_order = _format_pipeline_order(
                        schedule.pipeline_order
                    )
                    # print(formatted_pipeline_order)
                    _validate_pipeline_order(
                        schedule.pipeline_order, num_microbatches, num_stages, enable_zero_bubble=(i == 0),
                    )
