...
class TestLinalg(TestCase):
    ...
    def test_norm_bfloat16_and_half(self, device, dtype):
        ...
        def run_test_case(input_size, ord, keepdim):
            msg = (
                f"input_size={input_size}, ord={ord}, keepdim={keepdim}, dtype={dtype}"
            )
    ...
    @skipIf(
        ...
        "Numpy changed handling of degenerate inputs in 2.3.0",
    )
    @skipCUDAIfNoMagma
    @skipCPUIfNoLapack
    @dtypes(torch.float, torch.double, torch.cfloat, torch.cdouble)
    ...
    def test_eig_cuda_complex_eigenvectors(self, device, dtype):
        ...
        match_0_pos = torch.allclose(
            vals_complex[0], expected_val, atol=1e-5, rtol=1e-5
        )
        match_0_neg = torch.allclose(
            vals_complex[0], expected_val_conj, atol=1e-5, rtol=1e-5
        )
        match_1_pos = torch.allclose(
            vals_complex[1], expected_val, atol=1e-5, rtol=1e-5
        )
        match_1_neg = torch.allclose(
            vals_complex[1], expected_val_conj, atol=1e-5, rtol=1e-5
        )
        ...
        self.assertTrue(
            torch.allclose(vals_real.imag, torch.zeros_like(vals_real.imag), atol=1e-6)
        )
...
""")
        ...
        for t1, t2, t3, t4 in itertools.product([True, False], repeat=4):
            ...
            for c, beta in itertools.product((M, V, V.unsqueeze(0)), (0, 1)):
                ...
                self._test_addmm_addmv(
                    func, c, m1, m2, beta=beta, transpose_out=t4, activation=activation
                )
    ...
    def test_logaddexp_cpu_vs_cuda_complex(self, device):
        ...
        other_complex = torch.complex(torch.tensor(-float("inf")), torch.tensor(2.0))
        out_gpu = torch.logaddexp(input=input_complex, other=other_complex)
        out_cpu = torch.logaddexp(input=input_complex.cpu(), other=other_complex.cpu())
        self.assertEqual(out_gpu.cpu(), out_cpu)

        input_complex = torch.complex(torch.tensor(2.0), torch.tensor(float("inf")))
        other_complex = torch.complex(
            torch.tensor(float("inf")), torch.tensor(float("inf"))
        )
        out_gpu = torch.logaddexp(input=input_complex, other=other_complex)
